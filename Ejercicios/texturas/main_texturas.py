...
def main():
    ...
    model = Obj()
    model.load("./box_texturas.obj")

    # Creo un programa de shading y guardo la referencia en la variable gouraud
    # gouraud = createShader("./gouraud_vs.hlsl", "./gouraud_fs.hlsl")
    ...
    while not end:
        ...
        glVertexPointer(3, GL_FLOAT, 0, model.get_vertices())
        glNormalPointer(GL_FLOAT, 0, model.get_normales())
        # Paso la lista de coordenadas de textura para cada vertice
        glTexCoordPointer(2, GL_FLOAT, 0, model.get_texturas())
        ...
        glDrawArrays(GL_TRIANGLES, 0, len(model.get_vertices()))
